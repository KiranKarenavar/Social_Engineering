# Social Engineering Lab Project

## 📌 Overview
This repository contains my **Module 9 Cybersecurity Lab Project** focused on  
**Social Engineering, Phishing, Insider Threats, and Identity Theft**.

The project combines **theoretical understanding** with **hands-on exposure** to commonly used social engineering and phishing tools in a **controlled lab environment**.

---

## 🎯 Objective
To understand:
- How social engineering attacks exploit human psychology
- Different types of phishing attacks
- Insider threats and their impact
- Identity theft techniques
- Ethical use of social engineering tools for awareness and defense

---

## 📚 Topics Covered
- Social Engineering fundamentals  
- Types of Social Engineering attacks  
- Phishing lifecycle  
- Insider Threats  
- Identity Theft  
- Defensive measures  

---

## 🛠️ Tools Used (Educational Purpose Only)
- Koadic  
- SocialPhish 2.0  
- PyPhisher  
- Zphisher  
- SEToolkit (Social Engineering Toolkit)

---

## 📸 Screenshots
Screenshots included demonstrate:
- Tool interfaces
- Menu navigation
- Lab environment execution  

> ⚠️ All screenshots are taken in a **controlled lab environment**.  
> No real victims, credentials, or live attacks are involved.

---

## 🔐 Ethical Disclaimer
This project is strictly for **educational and cybersecurity awareness purposes**.  
All tools were used in a controlled lab environment.  
Misuse of these tools for illegal or unethical activities is strictly discouraged.

---

## 📈 Learning Outcomes
- Understanding human-based cyberattacks  
- Hands-on exposure to phishing frameworks  
- Awareness of insider threats  
- Ethical hacking documentation skills  

---

## 👤 Author
**Kiran Karenavar**  
Cybersecurity / Ethical Hacking Student  

---

## 🏁 Conclusion
Social engineering remains one of the most effective cyberattack techniques due to its reliance on human behavior. This lab reinforced the importance of cybersecurity awareness, ethical testing, and defensive strategies.
